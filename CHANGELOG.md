...
- **`pi-link --team` reports the agent team already present in a repository, and `pi-link --team-check` validates a declared composition.** A team is a set of agent profiles in one repository — a role, a model, and a working directory each — and until now every project re-invented the same launcher and the same conventions to describe it. `--team` discovers what exists rather than owning new artifacts: agent profiles, skills, session configs and launch scripts, reading each profile's frontmatter for name, role, model and autoloaded skills. **Two harnesses, two config dirs:** OMP resolves agent definitions from `.omp/agents/` (project) and `~/.omp/agent/agents/` (user); Pi resolves them from `.pi/agents/` and `~/.pi/agent/agents/`, plus pi-subagents' legacy `.agents/`. A profile loads only in the harness whose root it sits in, so each profile is labelled with the harness that resolves it (`[project omp]`, `[project pi]`) rather than a bare `[project]`, which would wrongly imply both load it. OMP does read `.agents`, but only for skills, rules, prompts, commands and `AGENTS.md` — never agent definitions — which is why that root cannot be described as plainly loadable. Profiles are reported as **inventory, not roster**: everything either harness can load is listed, and a role joins the team only by being declared in the manifest. It is strictly read-only — it reads directory trees and reports, and never writes a file, starts a terminal, or touches link state. `--team --json` emits the same report as JSON so a launcher or a CI step can consume the resolved role paths directly. `--team-check` validates an optional declaration at `.pi-link/team.json`: every referenced profile, prompt and config file must exist, required skills must be present, `hub.role` must name a real role, and declared terminal names must not collide. Required errors exit `1` and print `ERROR …`; optional problems print `WARN …` and exit `0`, so a missing optional skill is never presented as a startup failure. It also warns when a skill id appears in more than one skill root — the same skill present in both tracked source (`skills/`) and a local install (`.agents/skills/`, typically gitignored alongside `skills-lock.json`). That pair is the normal source-to-install relationship, so it is a warning rather than an error: it is worth knowing because an install not refreshed after source edits goes stale, and nothing else reports it. A declared profile that exists but sits outside every harness's agent roots likewise warns rather than failing, since a launcher can still pass it by path even though neither harness will load it as a subagent. Paths in a manifest are repo-relative and normalized against the repository root.

- **A profile in the right root can still never load, and `--team` now says so.** Both harnesses skip any agent definition whose frontmatter lacks `name` **or** `description` — pi-subagents skips such files outright, and OMP behaves identically (a name-only profile in `.omp/agents` does not appear in `omp --print`'s subagent list, while an otherwise identical file with a `description` does). A correct path is therefore not the same as a registered agent, and nothing else reports the difference. Discovery marks these `(INERT: missing description)` and exposes `loadable` in `--team --json`; `--team-check` warns when a declared role points at one, because a role that cannot register is a composition bug a launcher cannot see. This surfaced real cases: five profiles in a sibling project sat in `.omp/agents` with a valid `name`, `role` and `model` but no `description`, so none of them had ever registered.

- **`pi-link --team-init` builds a manifest from discovery, and `pi-link --team-run` launches from one.** A manifest that only described a team drifted from whatever actually started it: a launcher that hardcodes each role's model, working directory and prompt path is a second description of the same team, and the two disagree silently. `--team-init` closes the first half — it builds `.pi-link/team.json` from discovered profiles, printing the proposal by default and writing only on `--write`, which refuses to overwrite an existing manifest. Paths come from discovery, the coordinator comes from `--hub`, and anything discovery cannot know (`cwd`, `sessionDir`, `config`) is omitted rather than invented so `--team-check` reports the gap instead of the manifest asserting a guess. Roles are keyed by a profile's declared `linkName`, so a file named `plantfluent-advisor.md` yields a role called `advisor`. User-level profiles are excluded, since their absolute paths are machine-local. `--team-run` closes the second half — it starts every declared role, reading the profile path, `cwd`, `sessionDir`, `config` and `linkName` from the manifest instead of deriving them from a naming convention, writes each profile body to `system-prompt.md` with its frontmatter stripped, and passes it as `omp --system-prompt @file`, and carries `--model` from the manifest (falling back to the profile's own frontmatter) so a role runs on the model it declares rather than the harness default. `--dry-run` prints the resolved plan (paths, prompt size, argv) and exits nonzero on any error so a broken plan never half-launches; the hub starts first so it wins the link race. The `--team` and `--team-check` modes stay strictly read-only. This was not hypothetical: in one migrated repo a role's profile sat outside the naming convention its launcher assumed, so the role started from a 19-byte placeholder prompt instead of its 1.5 KB profile, indefinitely and without an error, while the manifest declared the correct path and nothing read it.
...
- **`--team-run` launches through `pi` as well as `omp`, and `--team-check` validates declared models.** The two harnesses do not share a flag surface: `omp` accepts `--link-name`, `--cwd` and `--config`, and `pi` rejects all three as unknown options, so a pi team could not be launched at all. The harness is now resolved per run — `--harness omp|pi`, defaulting to the harness the repo's profile roots belong to, so a repo whose profiles live only under `.pi/agents` is a Pi team — and the argv is built from what that harness accepts. pi reaches its directory through the spawn cwd and takes its link name through the extension's `PI_LINK_NAME` env handoff; a declared `config` it cannot take is reported rather than silently dropped. Declared models are validated against what a harness actually reports: omp's registry via `models --json`, plus pi's enabled models from its settings file. Neither list is hardcoded, because a hardcoded table rejects valid ids as soon as a provider adds a model. Comparison is normalized — provider prefix and `:suffix` stripped, fuzzy substring either way — since the harnesses spell one model differently (omp reports id `glm-5.3` with selector `ollama-cloud/glm-5.3`, pi enables `ollama/glm-5.3:cloud`) and omp resolves models by fuzzy match, so all such spellings are legitimate and none may warn. An unavailable registry yields `null` and skips the check rather than rejecting everything. Three defects were fixed along the way: the wrapper exited `0` when every role failed to start (the first non-zero child exit is now its exit code); the model cache was a module-level binding in the temporal dead zone when `--team-check` ran, throwing a `ReferenceError`; and the no-model warning blamed a profile for not being discovered when it had been.
